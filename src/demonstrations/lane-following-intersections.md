...
(lane-following-intersections)=
# Lane Following with Intersections

This chapter describes the `Lane Following with Intersections` demonstration for your Duckiebot.

```{needget}
Completed [](lane-following).
---
A Duckiebot driving autonomously in a Duckietown without other vehicles or obstacles.
```
...
(demonstration-lane-following-expected)=
## Expected results
<!--
```{admonition} Outcome of the lane following demonstration.
...
(demonstration-lane-following-duckietown-setup)=
...
- The lighting is "good": ideally white diffused light. This demonstration relies on images from the camera and
  color detections, so avoid colored lights, reflections or other conditions that may confuse or blind
  the on-board image sensor;


(demonstration-lane-following-duckiebot-setup)=
...
(demonstration-lane-following-pre-flight)=
...
(demonstration-lane-following-run)=
## demonstration instructions

(demonstration-lane-following-run-demonstration)=
### Start the lane following demonstration

To start the lane following (`LF`) demonstration:

```none
dts duckiebot demo --demo_name lane_following --duckiebot_name DUCKIEBOT_NAME --package_name duckietown_demos
```

It will take a minute for the demonstration to launch. You can check Portainer if all the containers started successfully, and their logs if any issues arise.

(demonstration-lane-following-drive)=
...
```{list-table} Lane following demonstration start and stop commands
:header-rows: 1
:name: table:lf-start-commands
...
You can regain control of the Duckiebot at any moment by stopping the demonstration using the (virtual) joystick. The demonstration can be resumed by pressing the start button.

(demonstration-lane-following-visualization)=
...
* In `rqt_image_view` by selecting the `/ROBOT_NAME/ground_projection_node/debug/ground_projection_image/compressed` you should see a top-down view of the detected segments:
    ...
    ```{figure} ../_images/demonstrations/lane_following/ground_projection_segments.png
    :width: 30em

    Line segments projected on the ground
    ```

(demonstration-lane-following-troubleshooting)=
## Troubleshooting

```{trouble}
The demonstration does not respond why pressing the start or stop buttons
---
...
* Check if you can manually drive the Duckiebot
  * Try re-launching `dts duckiebot keyboard_control DUCKIEBOT_NAME`
* Check if ROS messages are received on the robot on the `DUCKIEBOT_NAME/joy` topic
```
...
---
For this demonstration, there should not be any intersections in the city layout. Duckiebots will interpret intersections as "broken" lanes, perceiving less salient features, potentially compromising the state estimate hence causing the driving troubles.
```
...
---
This may be due to wrongly constructed lanes or bad Duckiebot calibrations. Fortunately, feedback control should take care of most of these problems.

While running the demonstration modify the PID controller gains from the base station through:
