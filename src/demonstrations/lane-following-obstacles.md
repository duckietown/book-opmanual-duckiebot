...
```{needget}
* Completed [](introduction-to-demonstrations).
* A Duckietown loop, as defined in [](book-opmanual-duckietown:book).
---
A Duckiebot driving autonomously in a Duckietown loop with obstacles.
```
...
where `ROBOT_NAME` is the name of your robot (virtual or real).
...
`ROBOT_NAME/tof_driver_node/front_center_tof/range`.
...
`packages/obstacle_detection/tof_obstacle_detection_node` folder (either `default.yaml` or `ROBOT_NAME.yaml`
if you have created it).

You may look at the `tof_obstacle_detection_node` source code (in `packages/obstacle_detection/src/tof_obstacle_detection_node.py`
